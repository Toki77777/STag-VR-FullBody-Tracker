...
- **`BridgeDriver/`** is the SteamVR OpenVR driver (submodule `Simple-OpenVR-Bridge-Driver`, branch `AprilTagTrackers`). It hosts the named pipe and creates/moves the SteamVR trackers. It must be installed into SteamVR's `drivers` folder (see `Installer/` and `BridgeDriver/install_driver.*`).
