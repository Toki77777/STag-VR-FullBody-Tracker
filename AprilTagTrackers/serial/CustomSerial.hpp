...
{
    Reflect::ForEach(value, [&](std::string_view subKey, const auto& subValue) { ctx.WriteKey(subKey, subValue); });
}

void ReadReflectable(auto& ctx, auto& value)
{
    Reflect::ForEach(value, [&](std::string_view subKey, auto& subValue) { ctx.ReadKey(subKey, subValue); });
}
...
template <>
struct Serial<cv::aruco::DetectorParameters>
{
    static void Parse(FileStorageReader& ctx, cv::aruco::DetectorParameters& value)
    {
        value.readDetectorParameters(ctx.GetReader());
    }
    static void Format(FileStorageWriter& ctx, const cv::aruco::DetectorParameters& value);
};
